...
func (pg *Postgres) SaveOrder(order *Order) error {
	...
	err = tx.QueryRow(
		context.Background(),
		`INSERT INTO wb_tech_l0.delivery ("name", "phone", "zip", "city", "address", "region", "email") 
	...
	err = tx.QueryRow(
		context.Background(),
		`INSERT INTO wb_tech_l0.payment (transaction, request_id, currency, provider, amount, payment_dt, bank, delivery_cost, goods_total, custom_fee)
	...
	for _, item := range order.Items {
		err = tx.QueryRow(
			context.Background(),
			`INSERT INTO wb_tech_l0.items (chrt_id, track_number, price, rid, name, sale, size, total_price, nm_id, brand, status)
	...
	err = tx.QueryRow(
		context.Background(),
		`INSERT INTO wb_tech_l0."order" (order_uid, track_number, entry, delivery_id, payment_id, locale, internal_signature, customer_id, delivery_service, shardkey, sm_id, cof_shard, date_create)
	...
	for _, itemId := range itemsId {
		_, err = tx.Exec(context.Background(), `INSERT INTO wb_tech_l0."orderItems" (order_id, item_id)
...
func (pg *Postgres) GetOrderById(orderId int64) (Order, error) {
	...
	err = tx.QueryRow(context.Background(), `SELECT order_uid, track_number, entry, delivery_id, payment_id, locale, internal_signature, customer_id, delivery_service, 
		shardkey, sm_id, cof_shard, date_create FROM wb_tech_l0."order" WHERE id = $1`, orderId).Scan(
		&order.OrderUID,
		&order.TrackNumber,
		&order.Entry,
		&deliveryId,
	...
	err = tx.QueryRow(context.Background(), `SELECT name, phone, zip, city, address, region, email FROM wb_tech_l0.delivery WHERE id = $1`, deliveryId).Scan(
		&order.Delivery.Name,
		&order.Delivery.Phone,
		&order.Delivery.ZIP,
		&order.Delivery.City,
	...
	err = tx.QueryRow(context.Background(), `SELECT transaction, request_id, currency, provider, amount, payment_dt, bank, delivery_cost, goods_total, custom_fee FROM wb_tech_l0.payment WHERE id = $1`, paymentId).Scan(
		&order.Payment.Transaction,
		&order.Payment.RequestId,
		&order.Payment.Currency,
		&order.Payment.Provider,
	...
	allItems, err := tx.Query(context.Background(), `SELECT item_id FROM wb_tech_l0."orderItems" WHERE order_id = $1`, orderId)
	...
	for allItems.Next() {
		...
		err = conn.QueryRow(context.Background(), `SELECT chrt_id, track_number, price, rid, name, sale, size, total_price, nm_id, brand, status FROM wb_tech_l0.items WHERE id = $1`, itemId).Scan(
			&item.ChrtId,
			&item.TrackNumber,
			&item.Price,
			&item.RId,
...
func (pg *Postgres) GetCacheStateInDB() (map[int64]Order, error) {
	...
	cacheState, err := tx.Query(context.Background(), `SELECT order_id FROM wb_tech_l0.cache_state`)
	if err != nil {
...
func (pg *Postgres) SaveCacheInDB(orderId int64) {
	err := pg.db.QueryRow(context.Background(), `INSERT INTO wb_tech_l0.cache_state (order_id) VALUES ($1)`, orderId).Scan()
...
func (pg *Postgres) DeleteCacheState() {
	...
	_, err := pg.db.Exec(context.Background(), "DELETE FROM wb_tech_l0.cache_state")
	if err != nil {
